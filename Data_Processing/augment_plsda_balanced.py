...
def _sph_harm(l, m, theta, phi):
    """Evaluate complex spherical harmonics across SciPy API versions.

    ``sph_harm_y`` was added after the SciPy version bundled with
    SlicerSALT.  The legacy ``sph_harm`` function uses the opposite angle
    order: azimuth first, polar second.  Keeping that conversion here makes
    augmentation reproducible in both environments.
    """
...
def evaluate_spharm_icosahedron(coeffs_list, L, thetas, phis):
    ...
    for l in range(L + 1):
        c = coeffs_list[idx]
        y = _sph_harm(l, 0, thetas, phis).real
        X += c[0] * y
        Y += c[1] * y
        Z += c[2] * y
        idx += 1
        ...
        for m in range(1, l + 1):
            ...
            y_comp = _sph_harm(l, m, thetas, phis)
            factor = np.sqrt(2)
...
def classify_subject(subject_name):
    ...
    is_left = subject_name.lower().startswith(('left_', 'lh_')) or '_lh' in subject_name.lower()
    
    # Healthy Control / Normal (0)
    if "_HEALTHY" in name_upper or "HEALTHY" in name_upper or "HFH_" in name_upper or "NORMAL" in name_upper:
        return "Healthy", 0
    
    if (is_left and 'RIGHT-TLE' in name_upper) or (not is_left and 'LEFT-TLE' in name_upper):
        return 'Contralateral TLE (class 0)', 0
    # Ipsilateral or unspecified TLE follows the coefficient extractor's target.
...
def main():
    ...
    for fpath in coef_files:
        ...
        if not coeffs or len(coeffs) != expected_len:
            continue
        if classify_subject(basename)[1] < 0:
            raise ValueError(f'Unknown label: {basename}. Supply labelled training inputs only.')

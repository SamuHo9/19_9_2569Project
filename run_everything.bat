...
set "ICP_REFERENCE_ARGS="
if defined HIPPO_ICP_REFERENCE set "ICP_REFERENCE_ARGS=--reference_template "%HIPPO_ICP_REFERENCE%""
if "%HIPPO_ICP_FIT_REFERENCE%"=="1" set "ICP_REFERENCE_ARGS=--fit_reference"
"%SLICER_EXE%" --no-main-window --no-splash --python-script "%PIPELINE_DIR%\ICP\ICP.py" --input_dir "%INPUT_DIR%" --output_dir "%OUTPUT_DIR%" %ICP_REFERENCE_ARGS%
if errorlevel 1 (
    echo [ERROR] ICP failed. Pipeline stopped.
    pause
    exit /b 1
)
...
"%SLICER_EXE%" --no-main-window --no-splash --disable-scripted-loadable-modules --python-script "%PIPELINE_DIR%\SPHARM\run_spharm_batch.py" --input_dir "%OUTPUT_DIR%\aligned_nifti" --output_dir "%OUTPUT_DIR%"
if errorlevel 1 (
    echo [ERROR] SPHARM failed or incomplete. Pipeline stopped.
    pause
    exit /b 1
)
